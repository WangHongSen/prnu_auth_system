...
def get_info_from_QRcode(QRcode_path):
    ...
    if data:     
        ...
        raise Exception("QRcode Invalid!Please use the QRcode generated by us!")
...
def get_fingerprint_from_db(username):
    ...
    c = conn.cursor()       
    try:
        cursor = c.execute("SELECT fingerprint from user_table where username=?",(username,))
        for row in cursor:     
...
def save_fingerprint_into_db(fingerprint,username):
    ...
    c = conn.cursor()
    try:
        c.execute("UPDATE user_table SET fingerprint=? where username=?",(fingerprint,username))
        conn.commit()
    except:
        return None
        #username does not exist
...
def save_encode_data_into_db(encode_data,data):
    ...
    conn = sqlite3.connect(dbname)
    c = conn.cursor()
    c.execute("insert into QRcode_table values(?,?)",(encode_data,data))
    conn.commit()
    conn.close() 
...
def get_decode_data(encode_data):
    ...
    c = conn.cursor()
    try:
        cursor = c.execute("SELECT data from QRcode_table where encode_data=?",(encode_data,))
        for row in cursor:     
            data = row[0]    
        return data
    except:
...
def add_new_user(username,password):        
    ...
    c = conn.cursor()
    try:
        c.execute("insert into user_table values(?,?,'To be completed')",(username,encode_password))
        conn.commit() 
        return True
    except:
        return None
...
def get_password_by_username(username):
    ...
    c = conn.cursor()
    try:
        cursor = c.execute("SELECT password from user_table where username=?",(username,))
        for row in cursor:     
            encoded_password_in_database = row[0]
...
def test():
    ...
    conn = sqlite3.connect(dbname)
    c = conn.cursor()
    c.execute("delete from user_table")
    conn.commit()   
    conn.close()
