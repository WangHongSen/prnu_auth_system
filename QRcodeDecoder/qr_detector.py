...
def extract_matrix(image, debug=False):
    ...
    output_size (Default None) The size of the picture in the output for debug reasons. 
                               None, if no pictures should be printed on the screen
    Returns:
    A np matrix with a boolean value for every point of the qr code
    '''
    ...
    def debug_collect_images():
        ...
        try:  # because of being called in case of exceptions, some variables may not be defined
            ...
            image_list.append((cv2.resize(image_qr, 
                                          (temp_warp_size, temp_warp_size), 
   ...
   # checking if size is enough for getting good values
    if any(cv2.contourArea(contours[mark]) < 10 for mark in marks):
        raise QrDetectorError(
            'Some of the detected Finder Pattern are to small!', debug_collect_images())

    # calculating the center of the contour of each pattern
    moments_list = (cv2.moments(contours[mark]) for mark in marks)
    unsorted_center_list = [np.array([moments['m10'], moments['m01']]) / moments['m00'] 
                            for moments in moments_list]

    # matching the finder pattern to the corners TL, TR, BL
    distance_patternlist_tuple_list = (
        (np.linalg.norm(unsorted_center_list[patternindex_triple[BL]] - 
                        unsorted_center_list[patternindex_triple[TR]]), 
         patternindex_triple) # generating a tuple of distance and the pattern triple

            for patternindex_triple in itertools.permutations(range(3)) # iterating through permutations of possible matchings
            if 0 < np.cross(
                unsorted_center_list[patternindex_triple[TR]] - 
                unsorted_center_list[patternindex_triple[TL]], # filtering for clockwise matchings (TL TR BL)
                unsorted_center_list[patternindex_triple[BL]] - 
                unsorted_center_list[patternindex_triple[TL]])) 
    ...
    def pattern_corner_generator():
        ...
        for contour, center in zip(pattern_contour_list , pattern_center_list):
            ...
            categorie_distance_point_triple_list = (
                ((np.cross(horizontal_vector, contour_point - center) > 0, 
                  np.cross(verticial_vector, contour_point - center) < 0), 
                 np.linalg.norm(contour_point - center), contour_point) 
                for [contour_point] 
                in contour)
            ...
            corner_selection_tuple_list = itertools.groupby(
                sorted(categorie_distance_point_triple_list, 
            ...
            corner_points_triple_list = (
                max(values, key = operator.itemgetter(1)) 
                for _ , values 
                in corner_selection_tuple_list)

            # removing the boolean tuple and the distance and storing only the corner
            # coordinates in a list
            corner_coordinate_list = [
                coordinates 
                for _ , _  , coordinates 
                in corner_points_triple_list]

            yield corner_coordinate_list
    
    # creating a 2D np matrix whose first index is the finder pattern and
    # the second index is the corner
    # [pattern index][corner index]
    # pattern indices are TL, TR, BL
    # corner indices are TL, TR, BL, BR
    pattern_corner_list = np.stack(pattern_corner_generator())
    
    # calculating the number of pixels in the clean qr code 
    # This must be very exact
    pattern_average = np.mean([
        np.linalg.norm(pattern_corner_list[i][j] - pattern_corner_list[i][k]) 
        for i in range(3) 
        for j, k in [(TL,TR),(BL,BR),(TL,BL),(TR,BR)]
        ])

    size_average = np.mean([
        np.linalg.norm(pattern_corner_list[TL][TL] - pattern_corner_list[TR][TR]),  
        np.linalg.norm(pattern_corner_list[TL][BL] - pattern_corner_list[TR][BR]), 
        np.linalg.norm(pattern_corner_list[TL][TL] - pattern_corner_list[BL][BL]), 
        np.linalg.norm(pattern_corner_list[TL][TR] - pattern_corner_list[BL][BR]),
        ])
    
    # the width and the height of finder pattern is 7.  Using the rule of three
    estimated_pixel_count = size_average / pattern_average * 7
    
    # only estimated_pixel_count of 4 * Version + 17 are allowed => rounding to this number
    version = int(round((estimated_pixel_count - 17) / 4)) 

    if not 0 < version <= 40:
        raise QrDetectorError(
            'Calculated version {} is not between 1 and 40!'.format(version), debug_collect_images())
    ...
    if version > 1: 
        ...
        estimated_alignment_center = (
            pattern_center_list[TR] + pattern_center_list[BL] - pattern_corner_list[TL][BR])

        # finding possible alignment pattern
        possible_alignment_index_list = (
            i for i in range(len(hierarchy)) if get_dept(i) == 3)
        ...
        # pattern
        possible_alignment_index_list_prefiltered = (
            i for 
            i in possible_alignment_index_list 
            if np.all(np.abs(contours[i][0][0] - estimated_alignment_center) < alignment_area_delta))
        
        # calculating the center of all possible finder pattern
        possible_alignment_moment_list = (
            cv2.moments(contours[i]) 
            for i 
            in possible_alignment_index_list_prefiltered)

        possible_alignment_centers = [
            np.array([moments['m10'], moments['m01']]) / moments['m00'] 
            for moments 
            in possible_alignment_moment_list
            ]
    ...
    if possible_alignment_centers: # if we found the alignment pattern at bottom right, use this
        ...
        _, br_alignment_center = min(
            (np.linalg.norm(center - estimated_alignment_center), center) 
            for center 
            in possible_alignment_centers)

        source = np.array(
            [
            pattern_corner_list[TL][TL], 
            pattern_corner_list[TR][TR], 
            br_alignment_center, 
            pattern_corner_list[BL][BL]
            ], dtype = np.float32)

        destination = np.array(
            [(0, 0), 
             (temp_warp_size, 0), 
             (temp_warp_size - temp_pixel_size * 6.5, temp_warp_size - temp_pixel_size * 6.5), 
             (0, temp_warp_size)], 
             dtype = np.float32)
        ...
        t = np.cross(
            pattern_corner_list[BL][BL] - pattern_corner_list[TR][TR], 
            bl_b_dif_horizontal
            ) / np.cross(tr_r_dif_vertical, bl_b_dif_horizontal)
    ...
    image_bigqr_notthresholded = cv2.warpPerspective(
        image_gray, warp_matrix, (temp_warp_size, temp_warp_size))	

    image_bigqr = cv2.adaptiveThreshold(
        image_bigqr_notthresholded, 255, 
        cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 
        temp_pixel_size * 10 + 1, 2)

    # resizing to the real amount of pixels and thresholding again
    qr_notresholded = cv2.resize(
        image_bigqr[0 : temp_warp_size, 0 : temp_warp_size],
       (pixelcount, pixelcount))

    a ,image_qr = cv2.threshold(qr_notresholded, 127, 255, cv2.THRESH_BINARY)
    
    # extracting the data
    inverted_data = np.asarray(image_qr, dtype = np.bool)

    if debug:
        return np.logical_not(inverted_data), debug_collect_images()
    else:
        return np.logical_not(inverted_data)