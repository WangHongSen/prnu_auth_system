...
def extract_int_list(np_array, start_bit, word_length, word_count):
    '''
    Extracts a np array of integers out of a np array of booleans by interpreting the booleans as bits.
    Keyword arguments:
    np_array -- the np array of booleans
    ...
    Returns:
    A np array of integers with the length word_count
    '''
    mydata = np_array[start_bit:(start_bit + word_count * word_length)].reshape(word_count, word_length)
    weight = 2 ** np.arange(word_length - 1, -1,  -1)
    return np.sum(weight * mydata, 1)

def extract_int(np_array, start_bit, word_length):
    '''
    Extracts a integer out of a np array of booleans by interpreting the booleans as bits.
    Keyword arguments:
    np_array -- the np array of booleans
    start_bit -- the index of the first bit to be extracted
    ...
    '''
    weight = 2 ** np.arange(word_length - 1, -1,  -1)
    mydata = np_array[start_bit:(start_bit + word_length)]    
    return np.sum(weight * mydata)

def extract_long(array):
    '''
    Extracts a long integer out of a np array of booleans by interpreting the booleans as bits.
    Keyword arguments:
    np_array -- the np array of booleans
    Returns:
    A long integer
...
def get_dataarea_indicator(version):
    '''
    Generates a np matrix of booleans which values indicate, 
    whether the field of a qr code with the same index contains data or a function pattern
    Keyword arguments:
    version -- the version of the qr code
    Returns:
    A np matrix of booleans which values are true if the field of a qr code with the same index contains data
    '''
    size = version * 4 + 17
    # generating a matrix of true booleans which are indicating the data area
    retval = np.ones((size,size), dtype=bool)   
    ...
    if version > 1 : # removing alignment pattern for every version containing these patterns
        ...
        alignment_position_generator_first_col = (
            (alignment_start, alignment_start_remaining + col_factor * alignment_distance) 
            for col_factor 
            in range(alignment_gaps_count - 1))

        alignment_position_generator_first_row = (
            (alignment_start_remaining + row_factor * alignment_distance, alignment_start) 
            for row_factor 
            in range(alignment_gaps_count - 1))
        
        # the alignment patterns always have the same gaps so they are created
        # in a nested loop (itertools.product)
        alignment_position_generator_remaining = (
            (alignment_start_remaining + row_factor * alignment_distance, 
            alignment_start_remaining + col_factor * alignment_distance) 
            for row_factor, col_factor 
            in itertools.product(range(alignment_gaps_count), repeat = 2))

        # chaining all generators of alignment patterns to one
        alignment_position_generator = itertools.chain(
            alignment_position_generator_first_col, 
            alignment_position_generator_first_row, 
            alignment_position_generator_remaining)
...
def get_version_size(bit_matrix):
    ...
    Returns:
    A tuple containing the version of the qr code (1-40) and 
       the length of a side of the squared associated data matrix
    '''
...
def get_version_info(bit_matrix):
    ...
    bit_matrix -- the boolean data matrix of the qr code
    Returns:
    A np array of booleans with 18 fields containing the version information
    '''
    version, size = get_version_size(bit_matrix)
    return np.transpose(bit_matrix[size - 9 : size - 12: -1, 5::-1]).flatten() if version >= 7 else None
...
def get_format_info_data(bit_matrix):
    ...
    '''
    format_info = np.append(bit_matrix[8,[0, 1, 2, 3, 4, 5, 7,8]], 
                               bit_matrix[[7, 5, 4, 3, 2, 1, 0], 8])

    format_info = np.logical_xor(
...
def extract_bit_array(bit_matrix, mask_index, debug=False):
    ...
    # creating the raw mask_matrix
    mask_matrix = np.fromfunction(MASK_FUNCTIONS[mask_index], bit_matrix.shape, dtype = int)
    # removing the parts which contain no data
    mask_matrix = np.logical_and(mask_matrix, dataarea_indicator, mask_matrix)  
    # inverting the pixels of the original image, which are indicated by the mask
    bit_matrix_unmasked = np.logical_xor(bit_matrix, mask_matrix) 
    ...
    offset_indexlist_tuple_gen_right = zip(
        range(size - 1, 7, -2), 
        itertools.cycle([index_column_gen_up, index_column_gen_down]))

    offset_indexlist_tuple_gen_left  = zip(
        range(5,        0, -2), 
        itertools.cycle([index_column_gen_down, index_column_gen_up]))

    offset_indexlist_tuple_gen = itertools.chain(
        offset_indexlist_tuple_gen_right, offset_indexlist_tuple_gen_left) 
    ...
    indexlist_unfiltered = (
        (row, col - delta) 
        for col, column_gen 
        in offset_indexlist_tuple_gen 
        for delta, row 
        in column_gen)
    ...
    indexlist = ((row, col) 
                 for row, col 
                 in indexlist_unfiltered 
                 if dataarea_indicator[row, col])
...
def error_correction(raw_bit_array, version, ecc_level):
    ...
    short_codeword_block_index_list_gen = (
        range(block, short_codeword_block_bytes_count * block_count, block_count) 
        for block in range(short_codeword_block_count))

    long_codeword_block_index_list_gen = (
        itertools.chain(range(block, short_codeword_block_bytes_count * block_count, block_count), 
    ...
    codeword_block_index_list_gen = itertools.chain(
        short_codeword_block_index_list_gen, long_codeword_block_index_list_gen)
    ...
    errorcorrection_block_index_list_gen = (
        range(codeword_count + block, bytes_count, block_count) 
        for block in range(block_count))

    # extracting the data and the correction data for each block
    codeword_errorcorrection_block_index_list_gen = (
        list(itertools.chain(block, correction_data)) 
        for block, correction_data 
        in zip(codeword_block_index_list_gen, errorcorrection_block_index_list_gen))

    raw_byte_array = np.packbits(raw_bit_array)  # converting the array of bits to arrays of bytes

    # applying reed Solomon error-correction to all blocks
    # the result is a generator of np arrays of corrected bytes
    corrected_byte_list_gen = (
        reedsolo.rs_correct_msg(raw_byte_array[codeword_errorcorrection_block_index_list], 
                                errorcorrection_block_bytes_count) 
        for codeword_errorcorrection_block_index_list 
        in codeword_errorcorrection_block_index_list_gen)
    ...
    corrected_byte_array = np.fromiter(
        ...
        dtype=np.uint8, count=codeword_count)
        ...
        while mode_value in SUPPORTED_MODES: #TODO: testing every mode, mixed modes
            ...
            if mode_index == 0: # numeric mode - words of 10 bits contain 3 chars (0 - 9)
                ...
                if remaining_chars_count > 0:
                    ...
                    # of 4 or 8 bits
                    remaining_ints = extract_int(
                        corrected_bit_array, remaining_chars_start, remaining_chars_count * 4)
            ...
            # alphanumeric mode - words of 11 bits contain 2 chars (0 - 9, A - Z, $, %, *, +, -, ., /, :)
            elif mode_index == 1: 
                CHAR_LOOKUP = [str(i) for i in range(10)] + \
                   [chr(65 + i) for i in range(26)] + list(' $%*+-./:')
            ...
            # byte mode - words of 8 bits contain 1 char (ASCII)
            elif mode_index == 2:
